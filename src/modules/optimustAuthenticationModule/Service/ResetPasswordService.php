...
use Optimust\Authentication\Dao\ResetPasswordDao;
...
use Optimust\Core\Utility\Base64Url;
...
use Optimust\ORM\Exception\TransactionException;
...
class ResetPasswordService
{
    ...
    public function generateEmailBody(string $templateFile, array $placeholders, array $replacements)
    {
        $body = file_get_contents(
            Config::get(
                Config::MODULES_DIR
            ) . '/optimustAuthenticationModule/config/data/' . $templateFile
        );
     ...
     */
    protected function generatePasswordResetEmailBody(string $resetCode, string $userName)
    {
     ...
     * @return bool
     */
    public function sendPasswordResetCodeEmail(string $resetCode, string $userName): bool
    {
        try {
            $this->getEmailService()->setMessageTo([$userName]);
            $this->getEmailService()->setMessageFrom(
                [$this->getEmailService()->getEmailConfig()->getSentAs() => 'Optimust']
            );
            $this->getEmailService()->setMessageSubject('Optimust Password Reset');
            $this->getEmailService()->setMessageBody(
                $this->generatePasswordResetEmailBody($resetCode, $userName)
            );
    ...
    public function logPasswordResetRequest(User $user): bool
    {
        ...
        $resetPassword->setResetEmail($user->getUserName());
        ...
        $emailSent = $this->sendPasswordResetCodeEmail($resetCode, $user->getUserName());
        if (!$emailSent) {
        ...
        try {
            ...
            if ($this->validateUser($user) instanceof User) {
                ...
                $success = $this->getResetPasswordDao()
                    ->updateResetPasswordValid($user->getUserName(), 0);
            }
