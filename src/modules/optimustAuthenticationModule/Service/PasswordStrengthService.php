...
use Optimust\Authentication\Dao\EnforcePasswordDao;
...
class PasswordStrengthService
{
    ...
    public function checkPasswordPolicies(UserCredential $credential, int $passwordStrength): array
    {
        ...
        if ($this->checkMinPasswordLength($password)) {
            $messages[] = 'Should have at least ' . $this->getMinLength() . ' characters';
        }
        if ($this->checkMaxPasswordLength($password)) {
            $messages[] = 'Should not exceed ' . $this->getMaxLength() . ' characters';
        }
        if ($this->checkMinLowercaseLetters($password)) {
            $messages[] = 'Your password must contain minimum ' . $this->getMinNoOfLowercaseLetters() . ' lower-case letters';
        }
        if ($this->checkMinUppercaseLetters($password)) {
            $messages[] = 'Your password must contain minimum ' . $this->getMinNoOfUppercaseLetters() . ' upper-case letters';
        }
        if ($this->checkMinNumbersInPassword($password)) {
            $messages[] = 'Your password must contain minimum ' . $this->getMinNoOfNumbers() . ' numbers';
        }
        if ($this->checkMinSpecialCharacters($password)) {
            $messages[] = 'Your password must contain minimum ' . $this->getMinNoOfSpecialCharacters() . ' special characters';
        }
