...
use Optimust\Admin\Service\EmailConfigurationService;
use Optimust\Authentication\Auth\User as AuthUser;
use Optimust\Config\Config;
use Optimust\Core\Dao\EmailDao;
use Optimust\Core\Exception\CoreServiceException;
use Optimust\Core\Exception\DaoException;
use Optimust\Core\Exception\ServiceException as Exception;
use Optimust\Core\Mail\AbstractRecipient;
use Optimust\Core\Mail\MailProcessor;
use Optimust\Core\Mail\TemplateHelper;
...
use Optimust\Core\Utility\Mailer;
use Optimust\Core\Utility\MailMessage;
use Optimust\Core\Utility\MailTransport;
...
class EmailService
{
    ...
    public function getMessage()
    {
        ...
        if (empty($this->messageFrom)) {
            ...
            $this->messageFrom = [$this->getEmailConfig()->getSentAs() => "Optimust"];
        }
    ...
    public function sendTestEmail(string $toEmail): bool
    {
        ...
        $body = "This email confirms that $type details set in Optimust ";
    ...
    {
        $path = Config::get(Config::MODULES_DIR) . $path;
