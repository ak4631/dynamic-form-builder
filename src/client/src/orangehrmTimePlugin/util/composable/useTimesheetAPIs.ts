...
export default function useTimesheetAPIs(http: APIService) {
  ...
  const fetchTimesheet = (
    ...
    return http.request({
      method: 'GET',
      url: '/api/v1/time/timesheets/default',
      params: {
        date,
        empNumber,
      },
  ...
  const updateTimesheet = (
    ...
    return http.request({
      method: 'PUT',
      url: empNumber
        ? `/api/v1/time/employees/${empNumber}/timesheets/${timesheetId}`
        : `/api/v1/time/timesheets/${timesheetId}`,
      data: {
        action,
      ...
      http
        .request({
          ...
          url: isEmployeeTimesheet
            ? `/api/v1/time/employees/timesheets/${timesheetId}/entries`
            : `/api/v1/time/timesheets/${timesheetId}/entries`,
        })
