...
export default {
  ...
  setup(props) {
    const apiPath = props.employeeId
      ? `/api/v1/attendance/employees/${props.employeeId}/records`
  ...
  beforeMount() {
    ...
    this.setCurrentDateTime()
      .then(() => {
        ...
        let url = '/api/v1/attendance/records/latest';
        if (this.employeeId) {
          url = `/api/v1/attendance/records/latest?empNumber=${this.employeeId}`;
        }
  ...
  methods: {
    ...
    setCurrentDateTime() {
      return new Promise((resolve, reject) => {
        this.http
          .request({method: 'GET', url: '/api/v1/attendance/current-datetime'})
          .then((res) => {
