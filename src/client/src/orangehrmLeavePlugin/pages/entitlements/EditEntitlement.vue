...
export default {
  ...
  setup() {
    const http = new APIService(
      ...
      '/api/v1/leave/leave-entitlements',
    );
    http.setIgnorePath(
      '/api/v1/leave/leave-entitlements/[0-9]+/validation/entitlements',
    );
  ...
  beforeMount() {
    this.isLoading = true;
    this.http
      .request({method: 'GET', url: '/api/v1/leave/leave-periods'})
      .then(({data}) => {
      ...
      return new Promise((resolve) => {
        if (!isNaN(entitlement)) {
          this.http
            .request({
              method: 'GET',
              url: `/api/v1/leave/leave-entitlements/${this.entitlementId}/validation/entitlements`,
              params: {
                entitlement,
              },
            })
