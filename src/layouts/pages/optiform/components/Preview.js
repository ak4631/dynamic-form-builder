...
import { Dialog, DialogTitle, DialogContent, Button, Box, MenuItem, Radio, RadioGroup, FormControlLabel, TextField, Checkbox, FormControl, InputLabel, Stack, FormHelperText, InputAdornment, Tooltip, Link, Paper } from '@mui/material';
...
const Preview = ({ open = false, onClose, elements, layout, edit = 0 }) => {
  ...
  const handleSubmit = () => { notify('Form Submitted...', 's') };
  ...
  //                 test_val: `${label_val}_${element.id}`,
  //                 test: "Hello" // this property won't come when save
  //               }}
  //               icon={{
    ...
    edit == 0 ?
      ...
      <SoftBox>
        <Paper >
          ...
          {edit == 1 ?
            // <SoftButton variant="contained" color="success" size="small" startIcon={<AddIcon />} onClick={handleSubmit} sx={{ m: 2 }}>
            //   Add
            // </SoftButton> 
            <SoftButton variant="contained" color="success" size="small" startIcon={<AddIcon />} sx={{ m: 2 }}>
              Add
            </SoftButton>
          : null}
        </Paper>
